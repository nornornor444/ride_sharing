...
// 1. Fetch the current passenger's preferences
...
// Default to 0 if they haven't set preferences yet
if (!$p_prefs) {
    $p_prefs = ['chatty_flag' => 0, 'silent_flag' => 0, 'music_flag' => 0];
}

// Check if form is submitted with women_driver_pref checkbox (for current ride)
$form_women_pref = isset($_POST['form_women_driver_pref']) ? 1 : 0;
$use_women_filter = ($step == '1' && $form_women_pref) ? 1 : 0;

// 2. Fetch drivers ALONG WITH their preferences using a LEFT JOIN
// Filter by gender if women_driver_pref is enabled
...
while($driver = mysqli_fetch_assoc($drivers_result)){
    // Calculate the Vibe Match Score
    $match_score = 0;
    $total_prefs = 3;
    
    // Treat NULLs from the database as 0 for safe comparison
    ...
    // Build a string of emojis for their vibe
    $vibes = [];
...
// Sort drivers so the highest vibe match is at the top of the dropdown!
...
if($step == '2' && $_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    // Check if passenger has an active subscription for this route
    ...
    // If they have a subscription, it's free. Otherwise, charge normal rate.
    if($has_subscription) {
        $total_price = 0;
        $your_share = 0;
...
// Step 3: Save booking & Fetch Emergency Contact
$contact_found = false;
$emergency_name = "";
$emergency_phone = "";
...
<div class="bg-white p-8 rounded shadow-md w-full max-w-lg">
    ...
    <?php elseif($step == '3'): ?>
        <!-- Booking done, show emergency button and rating form -->
...
<script>
// Real-time filtering of drivers based on women preference
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (womenCheckbox && driverSelect) {
        // Store original options with gender data
        ...
        // Function to update driver list based on checkbox state
